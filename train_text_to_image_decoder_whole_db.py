...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=None,
        required=True,
        help="A folder containing the inference data of segmented images for prior.",
    )
...
class DreamBoothDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        for concept in os.listdir(instance_data_root):
            if os.path.isdir(os.path.join(instance_data_root, concept)):
                self.instance_images_path += list(Path(os.path.join(instance_data_root, concept)).iterdir())

        self.subject_images_path = []
        for concept in os.listdir(subject_data_root):
            if os.path.isdir(os.path.join(subject_data_root, concept)):
                self.subject_images_path += list(Path(os.path.join(subject_data_root, concept)).iterdir())
...
def main():
    ...
    train_dataset = DreamBoothDataset(
        ...
        subject_data_root=args.subject_data_dir,
        instance_prompt=args.validation_prompts,
        class_data_root=None,
        class_prompt=None,
        class_num=None,
        # tokenizer=tokenizer,
